#!/usr/bin/env python3
"""Rhythmus-Signale für humanize, keine Befunde.

Das Skript misst deterministische Rhythmus- und Burstiness-Signale. Es liefert
ausdrücklich nur Verdachtsmomente. Die Cluster-Regel, die Modus-Wahl und jede
Ausnahme bleiben beim Modell oder beim Lektorat.

Welcher Text gemessen wird, entscheidet ``segment.py``, dieselbe Segmentierung
wie bei den Regelprüfungen. Damit fallen KI-Bild-Callouts, Code, Linkziele und
Frontmatter-Bezeichner heraus, ohne dass dieses Skript eine eigene, schwächere
Schutzlogik pflegen muss.

Abgeleitet aus humanizer-de (Martin Moeller, MIT). Nur Standardbibliothek.
"""
...
def strip_protected(text: str) -> str:
    """Nicht zu prüfende Bereiche ausblenden, gemeinsam mit den Regelprüfungen."""
    return mask(text)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="rhythm_lint.py",
        description="Misst Rhythmus- und Burstiness-Signale deutschsprachiger Texte.")
    parser.add_argument("paths", nargs="*", metavar="PFAD",
                        help="Markdown-Dateien oder Verzeichnisse.")
    parser.add_argument("--text", help="Rohtext statt Datei, für Rauchtests.")
    parser.add_argument("--scope", choices=["skill_doc", "user_text", "changed_passages"],
                        default="user_text")
    parser.add_argument("--mode", choices=["locker", "sachlich", "formal"],
                        default="sachlich")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.text is not None:
        berichte = [analyze(args.text, file="<text>", scope=args.scope, mode=args.mode)]
    elif args.paths:
        dateien: list[Path] = []
        for roh in args.paths:
            pfad = Path(roh)
            if not pfad.exists():
                print(f"FEHLER: {pfad} gibt es nicht", file=sys.stderr)
                return 2
            dateien.extend(sorted(pfad.rglob("*.md")) if pfad.is_dir() else [pfad])
        if not dateien:
            print("Keine Markdown-Datei gefunden.")
            return 0
        berichte = []
        for datei in dateien:
            try:
                ...
                return 2
            berichte.append(analyze(roh, file=str(datei), scope=args.scope,
                                    mode=args.mode))
    else:
        print("FEHLER: kein Pfad angegeben.", file=sys.stderr)
        return 2

    ausgabe = berichte[0] if len(berichte) == 1 else berichte
